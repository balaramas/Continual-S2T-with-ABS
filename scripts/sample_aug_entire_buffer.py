...
def lbs(x_train, dataset, data_augmentation_dir, curr_task):
    ...
    # TODO Data Augmentation on buffer samples

    replay_data = replay_data.drop_duplicates(subset='tgt_text', keep=False)

    replay_data = augment(replay_data, data_augmentation_dir)
...
if __name__ == "__main__":
    ...
    with open(x_train_file[:-6] + 'lbs_aug_entire_buffer_st.tsv', 'w') as train_x_lbs_st_file:
        train_x_lbs_st_file.write(train_x_lbs.to_csv(sep='\t', index=False))
    with open(x_train_file[:-6] + 'rrs_aug_entire_buffer_st.tsv', 'w') as train_x_rrs_st_file:
